...
public class BoardManager {
	...
	BoardManager() {
		...
		for (int i = 0; i < this.boards.length; ++i) {
			...
			Team x = boards[i].registerNewTeam("towncandy_npc"); //issues if apiManager player "towncandy_npc" shows up
	...
	public boolean isGhosting(final Player p) {
		...
		for (String entry : t.getEntries()) {
			if (entry.equals(myTeamCode)) {
				return false; //player is in its home team
			}
		}
	...
	public boolean toggleShowingHealth(final Player p) {
		...
		BetterTeams.getMain().getLogger().info(b.toString()); // TODO: Debug
	...
	public boolean toggleHideNameplates(final Player p) {
		...
		if (b == this.boards[4]) { //no longer hiding nameplates
			...
			if (BetterTeams.apiManager.showMCNames(p)) {
				this.toggleShowingRPNames(p);
			}

			if (BetterTeams.apiManager.showHealth(p)) {
				this.toggleShowingHealth(p);
			}
			return false;
	...
	void createTeams(Affixes a) {
		...
		if (BetterTeams.apiManager.hideNameplates(p)) {
			this.toggleHideNameplates(p);
		}
